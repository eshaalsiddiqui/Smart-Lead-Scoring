...
def score_leads(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['status'] = df['next_best_action'].map({'Call': 'high', 'Email': 'medium', 'Nurture': 'low'})
    return df
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        categorical_columns = metadata.get('categorical_columns') or [
            ...
            if c in ['industry', 'company_size', 'region', 'source_channel', 'last_touch', 'job_title']
        ]
        ...
        if LEADS_DATA_PATH.exists():
            ...
            logger.warning(f"{LEADS_DATA_PATH} not found - /leads and /analytics endpoints will be empty")
...
async def predict_batch_leads(request: BatchPredictionRequest, model=Depends(get_model)):
...
async def get_top_leads(
    ...
    ].sort_values('revenue_impact', ascending=False).head(limit)
...
async def get_analytics_summary(leads: pd.DataFrame = Depends(get_leads_cache)):
    ...
    conversion_rate = float(leads['converted'].mean()) if 'converted' in leads.columns else float(leads['conversion_probability'].mean())
    ...
    monthly_trend = monthly_trend[['month', 'leads', 'conversions']].tail(12).to_dict('records')
...
async def chatbot_query(request: ChatRequest, leads: pd.DataFrame = Depends(get_leads_cache)):
    ...
    if 'top' in message and 'lead' in message or 'best lead' in message:
        ...
        return ChatResponse(response="Here are your top 5 leads by revenue impact:\n\n" + "\n".join(lines))
    ...
    if 'industry' in message or 'sector' in message:
        ...
        return ChatResponse(response="Industry breakdown (by lead volume):\n\n" + "\n".join(lines))

    if 'action' in message or 'next step' in message or 'attention' in message:
        action_counts = leads['next_best_action'].value_counts()
        urgent = leads[leads['next_best_action'] == 'Call'].sort_values('revenue_impact', ascending=False).head(3)
        urgent_names = ", ".join(urgent['company_name'].tolist()) if not urgent.empty else "none"
        return ChatResponse(response=(
