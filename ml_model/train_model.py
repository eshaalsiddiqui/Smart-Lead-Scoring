...
class LeadScoringModel:
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        logger.info(
            f"Prepared {len(X)} samples with {len(self.feature_columns)} features"
        )
    ...
    def save_model(self, model_path: str = "models"):
        ...
        metadata = {
            ...
            "auc_score": (
                float(self.best_auc_score) if self.best_auc_score is not None else None
            ),
...
def main():
    ...
    for name, result in results.items():
        ...
        print(
            f"  CV AUC: {result['cv_scores'].mean():.4f} ± {result['cv_scores'].std():.4f}"
        )
        print()
