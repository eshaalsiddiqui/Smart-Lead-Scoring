...
def transform_leads(raw_leads: pd.DataFrame) -> pd.DataFrame:
    ...
    df['lead_quality'] = np.where(
        df['decision_maker'] == True, 'High',
        np.where(df['engagement_score'] > df['engagement_score'].quantile(0.7), 'Medium', 'Low')
    )
    ...
    logger.info(f"Transformed {len(df)} leads with {df['data_quality_score'].sum()} high-quality records")
    return df
...
def generate_lead_insights(transformed_leads: pd.DataFrame) -> Dict:
    ...
    insights = {
        ...
        'top_industry': transformed_leads['industry'].mode().iloc[0] if len(transformed_leads) > 0 else None,
        'top_region': transformed_leads['region'].mode().iloc[0] if len(transformed_leads) > 0 else None,
    ...
    logger.info(f"Generated insights: {insights['total_leads']} leads, "
...
@flow
def backfill_historical_data(start_date: datetime, end_date: datetime, num_leads_per_day: int = 30):
