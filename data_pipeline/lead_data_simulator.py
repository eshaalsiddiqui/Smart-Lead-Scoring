...
class LeadDataSimulator:
    ...
    def generate_lead_data(
        self, num_leads: int = 1000, start_date: datetime = None
    ) -> pd.DataFrame:
        ...
        for i in range(num_leads):
            ...
            company_size = np.random.choice(
                self.company_sizes, p=[0.20, 0.25, 0.20, 0.15, 0.10, 0.10]
            )
            ...
            emails_opened_30d = max(
                0, int(emails_sent * max(0, min(1, email_open_rate)))
            )
    ...
    def _calculate_conversion_probability(
        ...
        touch_impact = {
            ...
            "contact_form": 0.06,
            "webinar": 0.04,
            "content_download": 0.03,
            "product_tour": 0.02,
            "case_study_view": 0.01,
            "email_open": 0.01,
            "ad_click": 0.0,
        }.get(last_touch, 0)
...
if __name__ == "__main__":
    ...
    simulator.save_to_csv(
        daily_batch, f"data/daily_leads_{today.strftime('%Y%m%d')}.csv"
    )

    print(
        f"Generated {len(leads_df)} training leads and {len(daily_batch)} daily leads"
    )
